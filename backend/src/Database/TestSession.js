...
async function Insert(session)
{
	...
	try
	{
		...
		// concurrentInserts.push(
			...
			+ "VisualArtDescription, VideoGamesTimeSpent, VideoGamesDescription, Age, Gender) "
		...
		for (let t = 0; t < tests.length; t++)
		{
			...
			let result = await connection.query(
				"INSERT INTO Tests (SessionId, Type, Duration, UserConfidence, Metadata) "
...
async function GenerateSessionId(connection)
{
	...
	if (exists.rows.length)
		return GenerateSessionId(connection);
	else
...
function DemographicToTable(demographic)
{
	return {
		...
		WorkplaceGraphicsImportance: 0,
		WorkplaceGraphicsDescription: 0,

		WorkplaceDrawingsImportance: 0,
		WorkplaceDrawingsDescription: 0,

		VisualArtTimeSpent: 0,
		VisualArtDescription: 0,

		VideoGamesTimeSpent: 0,
		VideoGamesDescription: 0,
