import { Task, Option, TaskResult } from "../tasks";
import { UserInterface, TaskDisplay } from "../io";
import { Timer, UnlimitedTimer } from "../metrics";

export class ImageComparisonInstruction extends Task
{
	private display : TaskDisplay;

	constructor(image1:string, image2:string)
	{
		super();
		this.display = new ImageComparisonInstructionPage(this, image1, image2);

		this.SetTitle("Instructions");
		this.SetCofidenceTracked(true);
	}

	OptionSelected(selectedOptions: Option): void
	{
		this.Complete();
	}
	
	GetOptions(): Option[]
	{
		return [];
	}
	
	GetDisplay(): TaskDisplay
	{
		return this.display;
	}

	GetTimer(): Timer
	{
		return new UnlimitedTimer();
	}
}

class ImageComparisonInstructionPage extends TaskDisplay
{
	private template : string;
	private task : Task;

	constructor(task : Task, image1 : string, image2 : string)
	{
		super();
		this.task = task;
		this.template =
		`<div style="width: 80%">
			<div style="display: flex">
				<div style="flex:6"><img src="${image1}" /></div>
				<div style="display: flex;flex: 2; justify-content: center;align-items: center;">
					<span style="font-size: 20pt;">&gt;</span>
				</div>
				<div style="flex:6"><img src="${image2}" /></div>
			</div>
			<hr />
			Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to corporate strategy foster collaborative thinking to further the overall value proposition. Organically grow the holistic world view of disruptive innovation via workplace diversity and empowerment.
			<hr />
			<button id="begin" class="btn btn-primary">Begin</button>
		</div>`;
	}

	Display(screen: UserInterface): void
	{
		screen.ViewModeContent();
		screen.ContentContainer().html(this.template);

		$("#begin").click(() =>
		{
			this.task.Complete();
		});
	}
	
}